...
function resetCourseModalFields() {
    ...
    updateGrandparent(regionSelect, false);
    city.value = '';
    updateGrandparent(city, false);
    name.value = '';
    while (coursesTable.children.length > 0) coursesTable.children[0].remove();
    updateGrandparent(coursesTable, false);
};
...
async function searchCourse() {
    ...
    const { data, message, success } = JSON.parse(response);
    ...
    if (newCourses.length > 0) {
        ...
        updateGrandparent(coursesTable);
    } else {
        feedbackParent.classList.remove('d-none');
        feedbackParent.removeAttribute('visibility');
    };
...
//     const course = courses.find(({ randa, _id }) => _id == this.value || (/^randa/.test(this.value) && randa == this.value.split('randa-')[1]));
...
//                         classList: ['text-center'],
//                         attributes: [{ id: 'id', value: `${teeValue}-${i}|${property}` }]
//                     });
//                 } else {
//                     dataElementObject.children.push({
...
//                 const id = `${teeValue}-${hole.index}|`;
//                 const distance = document.getElementById(`${id}distance`);
//                 const par = document.getElementById(`${id}par`);
//                 const strokeIndex = document.getElementById(`${id}strokeIndex`);
//                 if (distance) {
//                     if (distance.tagName === 'INPUT') distance.setAttribute('value', hole.distance);
//                     else distance.innerText = hole.distance;
//                 };
//                 if (par) {
//                     if (par.tagName === 'INPUT') par.setAttribute('value', hole.par);
//                     else par.innerText = hole.par;
//                 }
//                 if (strokeIndex) {
//                     if (strokeIndex.tagName === 'INPUT') strokeIndex.setAttribute('value', hole.strokeIndex);
//                     else strokeIndex.innerText = hole.strokeIndex;
//                 };
//             };
//         };
...
function updateGrandparent(element, show = true) {
    const grandparent = element.parentElement.parentElement;
    if (show) {
        grandparent.classList.remove('d-none');
        grandparent.removeAttribute('visibility');
        return true;
    };
    grandparent.classList.add('d-none');
    grandparent.setAttribute('visibility', 'hidden');
    return false;
};
...
countrySelect.addEventListener('change', async function() {
    ...
    updateGrandparent(regionSelect);
});
...
regionSelect.addEventListener('change', function() {
    ...
    resetCourseSearchValidation();
    updateGrandparent(city, regionUnselected);
